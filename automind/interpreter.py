...
def exception_summary(e, working_dir, exec_file_name, format_tb_ipython):
    ...
    if format_tb_ipython:
        ...
        tb_str = "".join(
            [l for l in tb_lines if "automind/" not in l and "importlib" not in l]
        )
...
class RedirectQueue:
    def __init__(self, queue, timeout=5, strip_ansi=True):
        self.queue = queue
    ...
    def write(self, msg):
        try:
            ...
            if self.strip_ansi and isinstance(msg, str):
...
class Interpreter:
    ...
    def child_proc_setup(self, result_outq: Queue) -> None:
        ...
        # this seems to only be necessary because we're exec'ing code from a string,
        # a .py file should be able to import modules from the cwd anyway
        sys.path.append(str(self.working_dir))

        # capture stdout and stderr with ANSI stripping enabled
        # trunk-ignore(mypy/assignment)
        sys.stdout = sys.stderr = RedirectQueue(result_outq, strip_ansi=True)
    ...
    def _run_session(
        ...
        while True:
            ...
            try:
                ...
                result_outq.put(f"Error writing code to file: {e}")
                event_outq.put(("state:finished", "IOError", {"error": str(e)}, None))
                continue

            event_outq.put(("state:ready",))

            try:
                compiled_code = compile(code, self.agent_file_name, "exec")

                exec(compiled_code, global_scope)

                # Log some info about the global scope
                if logger.isEnabledFor(logging.DEBUG):
                    scope_summary = {
                        k: type(v).__name__ for k, v in list(global_scope.items())[:10]
                    }
            ...
            try:
                ...
                logger.error(f"Error removing code file: {e}")
        ...
        try:
            while not self.result_outq.empty() or not output or output[-1] != "<|EOF|>":
                try:
                    ...
                    if current_time - start_collect > 5:
                        logger.warning("Output collection timed out")
                        break
                    item = self.result_outq.get(timeout=1)
                    output.append(item)
                except queue.Empty:
                    continue
        except Exception as e:
            logger.error(f"Error while collecting output: {e}")

        if output:
            if output[-1] == "<|EOF|>":
                output.pop()  # Remove EOF
        else:
            logger.error("Output list is empty; no EOF marker received.")
